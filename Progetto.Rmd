...
```
Among the pathways enriched in the **glioma_CE versus control** comparison, both neuronal and immune-related signatures emerged prominently. *Neuroactive ligand-receptor interaction, Neuroactive signaling, and GABAergic synapse* were among the most represented categories, indicating substantial alterations in neuronal communication programs. Pathways involved in cytokine signaling, complement activation and immune-related interactions were also significantly enriched, suggesting the contribution of inflammatory processes and tumor microenvironment remodeling.  
Taken together, these findings support broad transcriptional alterations affecting both neural signaling and immune-associated mechanisms in glioma tissues.


```{r}
knitr::kable(ora_ne@result[1:10,c(4,5,8)], caption = "KEGG ORA results for glioma_NE vs control comparison")
```
A different enrichment profile was observed in the **glioma_NE versus control** comparison, where immune- and microenvironment-associated pathways became more prominent. *Cytokine–cytokine receptor interaction, Chemokine signaling*, and *ECM-receptor* interaction emerged among the most significantly enriched categories, indicating alterations in cell communication and tissue organization processes. The predominance of inflammatory and extracellular matrix signatures suggests an important contribution of tumor microenvironment-associated mechanisms within the non-enhancing region.


```{r}
knitr::kable(ora_ne_ce@result[1:10,c(4,5,8)], caption = "KEGG ORA results for glioma_NE vs glioma_CE comparison")
```
The KEGG enrichment analysis comparing **glioma_NE and glioma_CE** highlighted enrichment of neuronal and synaptic signaling pathways, including *Neuroactive ligand-receptor interaction, Synaptic vesicle cycle, Calcium signaling*, and several *neurotransmission-related pathways*. Since this comparison involves two tumor regions rather than tumor versus control samples, these findings likely reflect region-specific transcriptional differences. The predominance of neuronal signatures supports the presence of molecular heterogeneity across glioma compartments.
...
```
The biological processes identified through GO analysis largely mirrored the trends observed in KEGG enrichment. Biological processes related to neuronal communication and synaptic activity, including *regulation of trans-synaptic signaling, neurotransmitter transport, and modulation of chemical synaptic transmission*, represented the most significantly enriched categories. Immune-associated processes such as *humoral immune response* and *antimicrobial humoral response* were also enriched, indicating the coexistence of neuronal and inflammatory signatures in glioma tissues.


```{r}
knitr::kable(oraGO_ne@result[1:10,c(2,3,6)],caption = "GO ORA results for glioma_NE vs control")
```
In the *non-enhancing region*, the GO results showed a stronger prevalence of immune-associated biological processes. *Humoral immune response, antimicrobial response*, and *leukocyte migration* emerged among the most significantly enriched categories, suggesting a major contribution of inflammatory and immune-associated mechanisms within the non-enhancing region.


```{r}
knitr::kable(oraGO_ne_ce@result[1:10,c(2,3,6)],caption = "GO ORA results for glioma_NE vs glioma_CE comparison")
```
When the two glioma regions were compared directly, neuronal and synaptic processes represented the predominant biological signatures. Terms including *modulation of chemical synaptic transmission, regulation of trans-synaptic signaling*, and *neurotransmitter transport* appeared among the most significantly enriched categories. Additional terms related to membrane potential regulation and ion transport further support the existence of region-specific molecular differences between glioma compartments.
...
```
For the comparative KEGG enrichment analysis, several pathways appeared consistently across multiple comparisons, while others showed comparison-specific enrichment patterns. Neuronal and synaptic pathways, including *Neuroactive ligand-receptor interaction, GABAergic synapse, and Calcium signaling*, were represented across different contrasts, suggesting that alterations affecting neuronal communication constitute a recurrent feature of the analyzed samples. In contrast, pathways associated with *Cytokine signaling, ECM-receptor interaction*, and *transcriptional dysregulation* displayed stronger enrichment in the glioma_NE versus control comparison, indicating differences in biological processes across tumor regions.  
Similarly, the comparative GO enrichment analysis highlighted both common and comparison-specific biological processes. Terms related to neuronal communication, including *regulation of trans-synaptic signaling, neurotransmitter transport*, and *modulation of chemical synaptic transmission*, were recurrent across multiple comparisons. In contrast, immune-associated categories such as *humoral immune response* and *leukocyte migration* showed stronger enrichment in the glioma_NE versus control comparison.  
Overall, these results indicate that while some biological programs are shared across glioma regions, distinct transcriptional signatures characterize specific comparisons.
...
```
The UpSet plots provide an overview of the pathways and biological processes shared across comparisons as well as those uniquely associated with specific glioma regions. In the **glioma_CE versus control** comparison, both KEGG and GO analyses showed a strong representation of neuronal and synaptic signatures, including pathways and processes involved in neurotransmission and synaptic communication. Immune-associated categories were also detected, indicating the coexistence of neural and inflammatory components within tumor-associated transcriptional changes.

A different pattern emerged in the **glioma_NE versus control comparison**, where immune- and extracellular matrix-associated signatures became more prominent. Pathways related to cytokine signaling and extracellular matrix interactions, together with biological processes associated with immune responses and leukocyte activity, suggest a stronger contribution of microenvironment-associated mechanisms in the non-enhancing region.

Direct comparison between **glioma_NE and glioma_CE** samples highlighted the persistence of neuronal and signaling-related signatures, including processes involved in synaptic transmission and membrane potential regulation. These findings suggest that the two tumor regions retain distinct molecular characteristics and further support the existence of region-specific biological programs within glioma tissues.
...
```
The KEGG GSEA results highlighted coordinated pathway-level alterations across the three comparisons. In the **glioma_CE versus control** comparison highlighted strong enrichment of neuronal and synaptic-related pathways. *Long-term potentiation, Glutamatergic synapse*, and *Dopaminergic signaling* showed predominantly negative normalized enrichment scores (NES), indicating a coordinated reduction of neurobiological functions in tumor samples relative to controls. Cytokine-related pathways were also represented, suggesting the coexistence of neuronal and immune-associated alterations.

A similar profile was observed in the *glioma_NE versus control* comparison, where *Neuroactive ligand signaling, Long-term potentiation, Glutamatergic synapse*, and *GABAergic synapse* also displayed predominantly negative NES values. Compared with glioma_CE, the stronger negative scores suggest a more pronounced reduction of neuronal signatures within the non-enhancing region.

Direct comparison between *glioma_NE and glioma_CE* revealed a different pattern. Several neuronal and neurotransmission-related pathways displayed predominantly positive NES values, indicating relative enrichment of these signatures in the non-enhancing region.

Overall, the GSEA results confirm that transcriptional differences across glioma regions involve coordinated biological programs rather than isolated genes.
...
```
The GO-GSEA analysis highlighted a consistent reduction of neuronal and synaptic biological processes in both tumor versus control comparisons. Processes including synaptic vesicle exocytosis, neurotransmitter secretion, and signal release from synapses displayed predominantly negative normalized enrichment scores (NES) in both **glioma_CE versus control** and **glioma_NE versus control**, indicating coordinated depletion of neurobiological functions in tumor tissues. The stronger negative scores observed in glioma_NE suggest an even more pronounced reduction of these signatures within the non-enhancing region.

A different pattern emerged when comparing glioma regions directly. In the glioma_NE versus glioma_CE comparison, synaptic communication processes displayed positive NES values, indicating relative enrichment within the non-enhancing region, whereas *cytoskeleton- and cilium-associated processes*, including *axoneme assembly and microtubule bundle formation*, showed negative enrichment values and were more strongly associated with the contrast-enhancing region.

Overall, these findings indicate the presence of coordinated differences in biological programs across glioma compartments.
...
grafico_finale
```
The KEGG GSEA ridgeplots provide a global view of pathway enrichment patterns across glioma regions and control samples. In both **glioma_CE versus control** and **glioma_NE versus control** comparisons, neuronal and synaptic pathways, including *Long-term potentiation, Glutamatergic synapse, GABAergic synapse*, and *Neuroactive ligand signaling*, are predominantly shifted toward negative enrichment values. Since negative values in these comparisons correspond to control samples, these findings indicate a relative depletion of neuronal programs within tumor tissues.

In contrast, pathways associated with cytokine signaling and immune-related processes tend to shift toward positive values, suggesting increased representation of inflammatory and tumor-associated mechanisms.

A different pattern emerges when comparing the two tumor regions directly. Pathways involved in neuronal signaling, including *Neuroactive ligand signaling, GABAergic synapse, Glutamatergic synapse, Retrograde endocannabinoid signaling*, and *Serotonergic synapse*, display positive enrichment values, indicating a relative enrichment of these signatures within glioma_NE samples.

Overall, the ridgeplots support the existence of coordinated transcriptional differences across glioma regions and indicate that pathway-level alterations are driven by broad expression changes rather than isolated genes. In addition, the extended tails observed in some pathway distributions suggest the contribution of a limited number of genes displaying particularly strong expression changes, which may exert a stronger influence on the overall enrichment profile.


```{r fig.height=16, fig.width=8}
grafico_finale2
```
The GO GSEA ridgeplots provide a process-level view of the transcriptional alterations observed across glioma regions. In both **glioma_CE versus control** and **glioma_NE versus control** comparisons, processes associated with *vesicle trafficking, neurotransmitter release*, and *postsynaptic regulation* are predominantly shifted toward negative enrichment values, indicating reduced representation of physiological neuronal activities within tumor tissues.

Additional terms related to neuronal organization, including dendrite extension and postsynaptic regulation, follow similar patterns and suggest alterations affecting both functional and structural cellular processes.

Direct comparison between glioma regions revealed a distinct profile. Synaptic-related processes remain relatively enriched within glioma_NE samples, whereas processes associated with cellular organization and motility, including motile cilium assembly, cilium movement, and microtubule organization, show stronger association with glioma_CE tissues.

Overall, these findings indicate the presence of region-dependent biological differences across glioma compartments.



## Visualization of relevant pathways

We here aim to graphically represent the differential expression of critical pathways relevant to the considered conditions. To do so we'll exploit the amazing library of illustrated biological pathways provided by the KEGG database.
...
### Glioma pathway in glioma_ce vs control
...
#togliamo per non appesantire

```

### An unexpected result: go, grow... or do both?!

**glioma_ce vs control**
```{r,out.width="50%", fig.align='center'}
pathview(gene.data = fold_changes_ce, pathway.id = "hsa04110", species = "hsa",
         low = list(gene = "green"), mid = list(gene = "gray"), high = list(gene = "red"),
         out.suffix = "CE")
knitr::include_graphics("hsa04110.CE.png")
```


**glioma_ne vs control**
```{r,out.width="50%", fig.align='center'}
pathview(gene.data = fold_changes_ne, pathway.id = "hsa04110", species = "hsa",
         low = list(gene = "green"), mid = list(gene = "gray"), high = list(gene = "red"),
         out.suffix = "NE")
knitr::include_graphics("hsa04110.NE.png")
```
**glioma_ce vs glioma_ne**
```{r, out.width="50%", fig.align='center'}
pathview(gene.data = fold_changes_ne_ce, pathway.id = "hsa04110", species = "hsa",
         low = list(gene = "green"), mid = list(gene = "gray"), high = list(gene = "red"),
         out.suffix = "CENE")
knitr::include_graphics("hsa04110.CENE.png")
```

It's a famous conception in oncology (but also other fields such as developmental biology) that, since both migration and cellular division are really energetically expensive activities, a cell in migration will stop the cell cycle. We wanted to show this here, and we partially did by observing less upregulated cell cycle genes in glioma_NE vs control than in glioma_CE vs control; but at the end of the day, if we compare the solid CE portion of the tumor to the migrant NE portion we do not observe any relevant difference with the used methods.

### a filthy trick: inflammation (and excitotossicity)
...
By showing the glutammatergic synapse pathway in glioma_ce vs control we see (as already said multiple times) a elevate loss of synaptic function in the glioma. The loss of function might also be connected with the tumor exploiting glutammate excitotossicity. The upregulation of the PLA2's transcript (a lipase involved in inflammation) is on the other hand a clear sign of the tumor inducing an inflammatory state, that helps it grow.
...
pathview(gene.data = fold_changes_ne, pathway.id = "hsa04512", species = "hsa",
         low = list(gene = "green"), mid = list(gene = "gray"), high = list(gene = "red"),
         out.suffix = "CE")
knitr::include_graphics("hsa04512.CE.png")
```

```{r, out.width="80%", fig.align='center'}

pathview(gene.data = fold_changes_ne_ce, pathway.id = "hsa04512", species = "hsa",
         low = list(gene = "green"), mid = list(gene = "gray"), high = list(gene = "red"),
         out.suffix = "NE")
         

knitr::include_graphics("hsa04512.NE.png")
```

What we see is a clear and strong sign of migration via ECM in the glioma_NE. 

### Axon guidance in glioma_NE vs glioma_CE
...
The fact glioma_NE has upregulated axon guidance genes with respect to glioma_CE strongly enhances the statement that glioma_NE samples are mixed with healthy neuron cells.
